...
from steinerTree_creation import OPTICAL_REACH
REGENRESOURCESAVAILABLE=100
MAXDIFFATREG = 10 #including
...
def check_optical_reach_blocking_old(tree_path_dist, tree_path, regen_nodes,regen_node_limit, adj_mat):#FIXED NO OF REGEN NODES
    n_tree_path_dist = []
    n_tree_path = []
        ...
        if dist > OPTICAL_REACH:
            ...
            for j in range(len(path) - 1):
                ...
                if knowledge > OPTICAL_REACH :
                    ...
                    if(regen_node_limit[path[j]] == 0 or MAXDIFFATREG < max(regen_node_copy.values()) - min(regen_node_copy.values())):
